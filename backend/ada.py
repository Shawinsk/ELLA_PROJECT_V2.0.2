...
try:
    ...
    cv2 = None
    print("Warning: opencv-python not installed. Vision features generic disabled.")

try:
    import pyaudio
except ImportError:
    pyaudio = None
    print("Warning: pyaudio not installed. Checking for sounddevice...")
...
try:
    ...
    CadAgent = None
    print("Warning: CadAgent could not be imported.")

try:
    from web_agent import WebAgent
except ImportError:
    WebAgent = None
    print("Warning: WebAgent could not be imported.")

try:
    from kasa_agent import KasaAgent
except ImportError:
    KasaAgent = None
    print("Warning: KasaAgent could not be imported.")

try:
    from printer_agent import PrinterAgent
except ImportError:
    PrinterAgent = None
    print("Warning: PrinterAgent could not be imported.")
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_MODE, on_audio_data=None, on_video_frame=None, on_cad_data=None, on_web_data=None, on_transcription=None, on_tool_confirmation=None, on_cad_status=None, on_cad_thought=None, on_project_update=None, on_device_update=None, on_error=None, input_device_index=None, input_device_name=None, output_device_index=None, kasa_agent=None):
        ...
        self.cad_agent = CadAgent(on_thought=handle_cad_thought, on_status=handle_cad_status) if CadAgent else None
    ...
    def update_permissions(self, new_perms):
        print(f"[ELLA DEBUG] [CONFIG] Updating tool permissions: {new_perms}")
    ...
    def resolve_tool_confirmation(self, request_id, confirmed):
        print(f"[ELLA DEBUG] [RESOLVE] resolve_tool_confirmation called. ID: {request_id}, Confirmed: {confirmed}")
        if request_id in self._pending_confirmations:
            future = self._pending_confirmations[request_id]
            if not future.done():
                print(f"[ELLA DEBUG] [RESOLVE] Future found and pending. Setting result to: {confirmed}")
                future.set_result(confirmed)
            else:
                 print(f"[ELLA DEBUG] [WARN] Request {request_id} future already done. Result: {future.result()}")
        else:
            print(f"[ELLA DEBUG] [WARN] Confirmation Request {request_id} not found in pending dict. Keys: {list(self._pending_confirmations.keys())}")
    ...
    def clear_audio_queue(self):
        ...
        try:
            ...
            while not self.audio_in_queue.empty():
                ...
                count += 1
            if count > 0:
                print(f"[ELLA DEBUG] [AUDIO] Cleared {count} chunks from playback queue due to interruption.")
        except Exception as e:
            print(f"[ELLA DEBUG] [ERR] Failed to clear audio queue: {e}")
    ...
    async def listen_audio(self):
        if not pya and not sd:
            print("[ELLA] Audio disabled (PyAudio/SoundDevice not installed).")
            while True:
        ...
        if pya:
            ...
            if self.input_device_name:
                print(f"[ELLA] Attempting to find input device matching: '{self.input_device_name}'")
                    ...
                    try:
                        ...
                        if info['maxInputChannels'] > 0:
                            ...
                            if self.input_device_name.lower() in name.lower():
                                ...
                                print(f"[ELLA] Resolved input device '{self.input_device_name}' to index {i} ({name})")
                                break
                    except Exception:
                        continue
            ...
            input_idx = resolved_input_device_index if resolved_input_device_index is not None else mic_info["index"]
            ...
            try:
                ...
                print(f"[ELLA] [ERR] Failed to open audio input stream: {e}")
                return
        ...
        # --- SoundDevice Implementation ---
            ...
            print(f"[ELLA] Using SoundDevice for input.")
            resolved_index = None
            ...
            try:
                if self.input_device_name:
                    ...
                    for i, d in enumerate(devs):
                        if d['max_input_channels'] > 0 and self.input_device_name.lower() in d['name'].lower():
                            resolved_index = i
                            print(f"[ELLA] Resolved input to: {d['name']} ({i})")
                            break
            except Exception as e:
                print(f"[ELLA] Device lookup failed: {e}")
            
            if resolved_index is None and self.input_device_index:
                 try: resolved_index = int(self.input_device_index)
                 except: pass

            try:
                # Open Stream (Context Manager)
                with sd.RawInputStream(
                    ...
                    while True:
                         ...
                         # Read Blocking
                         data, overflow = await asyncio.to_thread(stream.read, CHUNK_SIZE)
                         data = bytes(data) # Convert CFFI buffer
                 ...
                 print(f"[ELLA] [ERR] SoundDevice Error: {e}")
                 # Prevent tight loop on error
                 while True: await asyncio.sleep(1)

    async def handle_cad_request(self, prompt):
        print(f"[ELLA DEBUG] [CAD] Background Task Started: handle_cad_request('{prompt}')")
        if self.on_cad_status:
        ...
        if self.project_manager.current_project == "temp":
            ...
            print(f"[ELLA DEBUG] [CAD] Auto-creating project: {new_project_name}")
            ...
            if success:
                ...
                try:
                    ...
                    print(f"[ELLA DEBUG] [ERR] Failed to notify auto-project: {e}")
        ...
        if cad_data:
            print(f"[ELLA DEBUG] [OK] CadAgent returned data successfully.")
            print(f"[ELLA DEBUG] [INFO] Data Check: {len(cad_data.get('vertices', []))} vertices, {len(cad_data.get('edges', []))} edges.")
            
            if self.on_cad_data:
                print(f"[ELLA DEBUG] [SEND] Dispatching data to frontend callback...")
                self.on_cad_data(cad_data)
                print(f"[ELLA DEBUG] [SENT] Dispatch complete.")
            ...
            try:
                ...
                print(f"[ELLA DEBUG] [NOTE] Sent completion notification to model.")
            except Exception as e:
                 print(f"[ELLA DEBUG] [ERR] Failed to send completion notification: {e}")

        else:
            print(f"[ELLA DEBUG] [ERR] CadAgent returned None.")
    ...
    async def handle_write_file(self, path, content):
        print(f"[ELLA DEBUG] [FS] Writing file: '{path}'")
        ...
        if self.project_manager.current_project == "temp":
            ...
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            new_project_name = f"Project_{timestamp}"
            print(f"[ELLA DEBUG] [FS] Auto-creating project: {new_project_name}")
        ...
        print(f"[ELLA DEBUG] [FS] Resolved path: '{final_path}'")
        ...
        print(f"[ELLA DEBUG] [FS] Result: {result}")
        try:
             await self.session.send(input=f"System Notification: {result}", end_of_turn=True)
        except Exception as e:
             print(f"[ELLA DEBUG] [ERR] Failed to send fs result: {e}")

    async def handle_read_directory(self, path):
        print(f"[ELLA DEBUG] [FS] Reading directory: '{path}'")
        try:
    ...
    async def handle_read_file(self, path):
        print(f"[ELLA DEBUG] [FS] Reading file: '{path}'")
        try:
            if not os.path.exists(path):
    ...
    async def handle_web_agent_request(self, prompt):
        print(f"[ELLA DEBUG] [WEB] Web Agent Task: '{prompt}'")
        ...
        print(f"[ELLA DEBUG] [WEB] Web Agent Task Returned: {result}")
        ...
        try:
             ...
             print(f"[ELLA DEBUG] [ERR] Failed to send web agent result to model: {e}")
                            ...
                            if transcript:
                                ...
                                if transcript != self._last_output_transcription:
                                    ...
                                    if delta:
                                        # Send to frontend (Streaming)
                                        if self.on_transcription:
                                             self.on_transcription({"sender": "ELLA", "text": delta})
                                        
                                        # Buffer for Logging
                                        if self.chat_buffer["sender"] != "ELLA":
                                            # Flush previous
                                            if self.chat_buffer["sender"] and self.chat_buffer["text"].strip():
                                                self.project_manager.log_chat(self.chat_buffer["sender"], self.chat_buffer["text"])
                                            # Start new
                                            self.chat_buffer = {"sender": "ELLA", "text": delta}
                                        else:
                                            # Append
                                            self.chat_buffer["text"] += delta
                        ...
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "list_smart_devices", "control_light", "discover_printers", "print_stl", "get_print_status", "iterate_cad"]:
                                ...
                                if not confirmation_required:
                                    print(f"[ELLA DEBUG] [TOOL] Permission check: '{fc.name}' -> AUTO-ALLOW")
                                    ...
                                    print(f"[ELLA DEBUG] [STOP] Requesting confirmation for '{fc.name}' (ID: {request_id})")
                                    ...
                                    print(f"[ELLA DEBUG] [CONFIRM] Request {request_id} resolved. Confirmed: {confirmed}")

                                    if not confirmed:
                                        print(f"[ELLA DEBUG] [DENY] Tool call '{fc.name}' denied by user.")
                                ...
                                if fc.name == "generate_cad":
                                    print(f"\n[ELLA DEBUG] --------------------------------------------------")
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call Detected: 'generate_cad'")
                                    print(f"[ELLA DEBUG] [IN] Arguments: prompt='{prompt}'")
                                    ...
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'run_web_agent' with prompt='{prompt}'")
                                    ...
                                    print(f"[ELLA DEBUG] [RESPONSE] Sending function response: {function_response}")
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'write_file' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'read_directory' path='{path}'")
                                    ...
                                    path = fc.args["path"]
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'read_file' path='{path}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'create_project' name='{name}'")
                                    ...
                                    name = fc.args["name"]
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'switch_project' name='{name}'")
                                    ...
                                    if success:
                                        ...
                                        print(f"[ELLA DEBUG] [PROJECT] Sending project context to AI ({len(context)} chars)")
                                        try:
                                            await self.session.send(input=f"System Notification: {msg}\n\n{context}", end_of_turn=False)
                                        except Exception as e:
                                            print(f"[ELLA DEBUG] [ERR] Failed to send project context: {e}")
                                    function_response = types.FunctionResponse(
                                        id=fc.id, name=fc.name, response={"result": msg}
                                    )
                                    function_responses.append(function_response)
                                    ...
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'list_projects'")
                                    ...
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'list_smart_devices'")
                                    ...
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'control_light' Target='{target}' Action='{action}'")
                                    ...
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'discover_printers'")
                                    ...
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'print_stl' STL='{stl_path}' Printer='{printer}'")
                                    ...
                                    printer = fc.args["printer"]
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'get_print_status' Printer='{printer}'")
                                    ...
                                    print(f"[ELLA DEBUG] [TOOL] Tool Call: 'iterate_cad' Prompt='{prompt}'")
                                    ...
                                    if cad_data:
                                        print(f"[ELLA DEBUG] [OK] CadAgent iteration returned data successfully.")
                                        
                                        # Dispatch to frontend
                                        if self.on_cad_data:
                                            print(f"[ELLA DEBUG] [SEND] Dispatching iterated CAD data to frontend...")
                                            self.on_cad_data(cad_data)
                                            print(f"[ELLA DEBUG] [SENT] Dispatch complete.")
                                        ...
                                        print(f"[ELLA DEBUG] [ERR] CadAgent iteration returned None.")
    ...
    async def play_audio(self):
        if not pya and not sd:
            while True:
                # Discard audio if no playback device
        ...
        if pya:
            ...
            with sd.RawOutputStream(
                ...
                while True:
                    ...
                    if self.on_audio_data:
                        self.on_audio_data(bytestream)
                    # Sounddevice write is blocking, so run in thread
                    await asyncio.to_thread(stream.write, bytestream)

    async def get_frames(self):
        if not cv2:
            print("[ELLA] Video disabled (OpenCV not installed).")
            while True:
                await asyncio.sleep(1)
    ...
    async def run(self, start_message=None):
        ...
        while not self.stop_event.is_set():
            try:
                print(f"[ELLA DEBUG] [CONNECT] Connecting to Gemini Live API...")
                async with (
                    ...
                    if not is_reconnect:
                        if start_message:
                            print(f"[ELLA DEBUG] [INFO] Sending start message: {start_message}")
                        ...
                        print(f"[ELLA DEBUG] [RECONNECT] Connection restored.")
                        # Restore Context
                        print(f"[ELLA DEBUG] [RECONNECT] Fetching recent chat history to restore context...")
                        ...
                        print(f"[ELLA DEBUG] [RECONNECT] Sending restoration context to model...")
                ...
                print(f"[ELLA DEBUG] [STOP] Main loop cancelled.")
                break
                
            except Exception as e:
                # This catches the ExceptionGroup from TaskGroup or direct exceptions
                print(f"[ELLA DEBUG] [ERR] Connection Error: {e}")
                
                if self.stop_event.is_set():
                    break
                
                print(f"[ELLA DEBUG] [RETRY] Reconnecting in {retry_delay} seconds...")
