...
@app.get("/")
def read_root():
    return {"message": "ELLA V2 Backend is running!"}

@app.get("/status")
def health_check():
    return {"status": "ok", "service": "ella-v2-backend"}
...
@app.get("/status")
async def status():
    return {"status": "running", "service": "E.L.L.A Backend"}

@sio.event
async def connect(sid, environ):
    print(f"Client connected: {sid}")
    await sio.emit('status', {'msg': 'Connected to E.L.L.A Backend'}, room=sid)
...
@sio.event
async def retry_auth(sid):
    print("[SERVER] Client requested auth retry/camera access.")
    if authenticator:
        asyncio.create_task(authenticator.start_authentication_loop())
...
async def start_audio(sid, data=None):
    ...
    if audio_loop:
        if loop_task and (loop_task.done() or loop_task.cancelled()):
             ...
             await sio.emit('status', {'msg': 'E.L.L.A Already Running'})
             return
    ...
    try:
        ...
        print("Emitting 'E.L.L.A Started'")
        await sio.emit('status', {'msg': 'E.L.L.A Started'})
        ...
        print(f"CRITICAL ERROR STARTING ELLA: {e}")
        import traceback
        traceback.print_exc()
...
async def stop_audio(sid):
    ...
    if audio_loop:
        ...
        audio_loop = None
        await sio.emit('status', {'msg': 'E.L.L.A Stopped'})
...
async def discover_printers(sid):
    ...
    if not audio_loop or not audio_loop.printer_agent:
        ...
        if saved_printers:
            ...
            await sio.emit('status', {'msg': "Connect to E.L.L.A to enable printer discovery"})
            return
